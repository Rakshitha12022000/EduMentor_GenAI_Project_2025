...
def show_login():
    ...
    if st.button("Login"):
        ...
        if isinstance(user, dict):
            ...
            st.session_state.user = user  # 🔥 Store Firebase user info (for email)

            # 🔥 After login, fetch full profile info from Firestore
            ...
            if profile_info:
                ...
                st.session_state.user_role = profile_info.get("role", "Student")  # fallback Student
            else:
                st.warning("⚠️ Could not load profile information.")

            st.success("Dashboard being loaded.........")
            st.rerun()
        else:
